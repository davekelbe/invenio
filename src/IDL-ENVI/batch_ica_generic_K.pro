FUNCTION batch_ica_generic_K, dir_upper
...
dir_temp = dir_upper
dir_source = '/Volumes/Scythica/Queue/'
;dir_source = '/dirs/grad/djk2312/Flattened/EMEL/'
dir_write = strmid(dir_upper,0,strlen(dir_upper)-1)+'-jpg/'
...
;    '_c' + c_Str + '_m' + r_str + '_b' + q_str +  '.jpg'
   ...
   dir_out = dir_write + cube + info_slash + 'jpg' + info_slash;
  ...
  ; Read cube text file 
  filepath_cubes = dir_upper + cube + info_slash + 'matlab' + info_slash + cube + '_' + 'cube.txt'
...
;    if all_exist then continue
    ...
    for b =0,set_num[s]-1 do begin
      ...
      filepath_image = dir_source + cube + info_slash + cube + band_no_space + '.tif'
      im = 0; clear memory 
    ...
    for r = 0,n_roi-1 do begin 
      ...
      ; Do ICA computation 
      filename_trans = dir_upper + cube + info_slash + 'envi/' + cube + '_c' + set_str + '_m' + roi_str + '.trans'
