; http://www.dfanning.com/tip_examples/write_csv_data.pro
; APPEND keyword added by Jonathan Greenberg <jgreenberg@arc.nasa.gov>, 25 March 2005.
...
PRO Write_CSV_Data, $ 
   ...
   Append=append, $         ; If 0 (default), a new file is written (or overwritten); if 1, data is appended to file.
...
; to a spreadsheet program.
   ...
   xsize = s[0]
   IF ndims EQ 2 THEN ysize = s[1] ELSE ysize = 1
   ...
   IF append THEN BEGIN
      OpenW, lun, filename, /Get_Lun, Width=width, /Append
   ENDIF ELSE BEGIN
      OpenW, lun, filename, /Get_Lun, Width=width
   ENDELSE

   ; Write the column header row, if available.
   IF (N_Elements(columnHeaders) NE 0) AND (append eq 0) THEN BEGIN
   ...
   sData[0:xsize-1, *] = sData[0:xsize-1, *] + comma
   PrintF, lun, sData
