...
for m = 1:n_m
    ...
    filepath_mask = sprintf('%s%s_parchment_mask.tif', subpath_matlab_dir{m}, m_name{m});
    ...
    for w = 1:n_w;
        ...
        if nVarargs > 0;
            ...
            ww = ww(1);
            docontinue = true;
            band_number = m_wavelength_file{m}{ww}(end-8:end-6);
            wavelength = m_wavelength_file{m}{ww}(end-16:end-10);
            filepath_tif = sprintf('%s%s+%s_%s.tif',...
                subpath_tiff_dir{m}, m_name{m}, band_number, wavelength);
            filepath_jpg = sprintf('%s%s+%s_%s.jpg',...
                subpath_jpg_dir{m}, m_name{m}, band_number, wavelength);
        ...
        I = double(imread(m_wavelength_filepath{m}{ww}));
        %I_maxval = max(I(:));
        ...
        mask = mask./max(mask(:));
        mask = logical(mask);
        imwrite(mask, m_path_upper_mask{m});
        end
