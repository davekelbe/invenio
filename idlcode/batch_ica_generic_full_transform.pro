FUNCTION batch_ica_generic_full_transform, dir_upper, dir_task
...
;dir_write = dir_upper;strmid(dir_upper,0,strlen(dir_upper)-1)+'-out' + info_slash
dir_write = '/Volumes/Scythica/Queue-transform/'
...
for s = 0,n_subdir-1 do begin
  ...
  if strpos(SelectedFolders, 'jpg') NE -1 then begin
    continue
  endif
  ...
  cube = strmid(SelectedFolders_noslash,ix_slash+1, strlen(SelectedFolders_noslash)-ix_slash)
    ...
    print, 'Folio ' + cube
    dir_roi = dir_upper+cube+info_slash+'envi'+info_slash
    dir_matlab = dir_upper+cube+info_slash+'matlab'+info_slash
  ...
  n_set = n_elements(set_start)
    ...
    for f = 0,n_f-1 do begin
      ...
      if strpos(filename_current, '.jpg') EQ -1 then begin
        continue
      endif
      ...
      filename_trans = dir_upper + cube + info_slash + cube + '+' + 'envi' + info_slash + cube + '_c' + task_cube + '_m' + task_mask + '.trans'

      
      ; Now apply this transformation  to each of the other folios 
      ;for sTx = 0,n_subdir-1 do begin
        sTx = s
        SelectedFoldersTx = subdir[sTx] + info_slash
        if strpos(SelectedFoldersTx, 'jpg') NE -1 then begin
          continue
        endif
        SelectedFolders_noslashTx = strmid(SelectedFoldersTx, 0, strlen(SelectedFoldersTx)-1)
        ix_slashTx = strpos(SelectedFolders_noslashTx, info_slash, /reverse_search)
        cubeTx = strmid(SelectedFolders_noslashTx,ix_slash+1, strlen(SelectedFolders_noslashTx)-ix_slash)

        dir_out_up = dir_write + cubeTx + info_slash
        if ~file_test(dir_out_up, /directory) then begin
          command = 'mkdir ' + dir_out_up;
          spawn, command
        endif
        dir_out_jpg = dir_write + cubeTx + info_slash + cubeTx + '+' + 'jpg' + info_slash;
        if ~file_test(dir_out_jpg, /directory) then begin
          command = 'mkdir ' + dir_out_jpg;
          spawn, command
        endif
        dir_out_tiff = dir_write + cubeTx + info_slash + cubeTx + '+' + 'tiff' + info_slash;
        if ~file_test(dir_out_tiff, /directory) then begin
          command = 'mkdir ' + dir_out_tiff;
          spawn, command
        endif      
      ; for each band
      ; read and add to multiband image I 
      dir_sourceTx = dir_upper + cubeTx + info_slash + cubeTx + '+tiff' + info_slash
      
      ;continue if all output images already exist 
      
      
      for b =0,n_bands-1 do begin
        band_no_space = strreplacef(bands[b], ' ', '')
        filepath_image = dir_sourceTx  + cubeTx + band_no_space + '.tif'
        im = 0; clear memory 
   ...
   ; for q_ix = 0,n_bands-1 do begin ; for each band 
        ...
        filepath_jpg = dir_out_jpg + cubeTx + $
          '_' + cube + '_c' + c_Str + '_m' + r_str + '_b' + q_str +  '.jpg'
        filepath_tif = dir_out_tiff + cubeTx + $
          '_' + cube + '_c' + c_Str + '_m' + r_str + '_b' + q_str +  '.tif'
        write_jpeg, filepath_jpg, ICA_jpg, order=1, quality=50
        write_tiff, filepath_tif, ICA_tif, /short
     ; endfor   ; band  
    ;endfor ; target subdirectory     
    endfor ; Image in folio 
endfor ; Folio
